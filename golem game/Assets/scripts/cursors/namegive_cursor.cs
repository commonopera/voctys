...
public class namegive_cursor : MonoBehaviour {

	//navigate and control menu to name golem
	//options in menu based on which souls have been acquired

	Rigidbody2D rb2d;
	int position = 1, i, whichselected;
	int [] inventory;
	public float leftx, lefty, namex, namey, waitx, waity, diax, diay;
	bool begin = true, repeat = true;
	public GameObject approval, switchsfx, nameswitchsfx, denysound, waitsfx;
	...
	void Update () {
		...
		if(repeat){
			...
			else if (PlayerPrefs.GetInt("sacrednames") != 0){
				PlayerPrefs.SetInt("selectedname", inventory[0]);
				whichselected = 0;
			}
		}
		if(position == 1){
			...
			if(Input.GetKeyDown("down")){
				whichselected += 1;
				if(whichselected >= PlayerPrefs.GetInt("sacrednames")){
					whichselected = 0;
				}
				PlayerPrefs.SetInt("selectedname", inventory[whichselected]);
				Instantiate(nameswitchsfx, transform.position, Quaternion.identity);
			}
			if(Input.GetKeyDown("up")){
				whichselected -= 1;
				if(whichselected < 0){
					whichselected = PlayerPrefs.GetInt("sacrednames") - 1;
				}
				PlayerPrefs.SetInt("selectedname", inventory[whichselected]);
				Instantiate(nameswitchsfx, transform.position, Quaternion.identity);
			}
		...
		if(position == 2){
			//name
			rb2d.MovePosition(new Vector2(transform.parent.position.x + namex,
			transform.parent.position.y + namey));
			...
			if(Input.GetKeyDown("space")){
				if(PlayerPrefs.GetInt("sacrednames") > 0){
					PlayerPrefs.SetInt("chosenname", PlayerPrefs.GetInt("selectedname"));
					PlayerPrefs.SetInt("sacrednames", (PlayerPrefs.GetInt("sacrednames") - 1));
					PlayerPrefs.SetInt(("name" + PlayerPrefs.GetInt("selectedname").ToString()), 0);
